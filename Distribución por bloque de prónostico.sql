...
CREATE   Procedure [dbo].[sp_CP_distribucionDMD_pronostico_AP_JGH]
as 
...
-------------------------------------------------------------------------------------------------------------------------
--select * from  CP_Pronóstico_AP_JGH
...
--SELECT sum (TiempoDemandaVariableSAP) as T_Var, sum(TiempoControlesSAP)as T_Cont, sum(CantidadDemandaVariableSAP) as Q_Variable, sum(CantidadControlesSAP) as Q_Controles, mes= datepart(month,FECHA) FROM #VW_Z_PERCENTILES1 where Centro='San Bernardo' group by datepart(month,FECHA)
-------------------------------------------------------------------------------------------------------------------------
...
*/
drop table if exists #prónostico_mensual
	select Centro, Mes, Controles=Controles/(Semanas_mes), Variable=Variable/(Semanas_mes)
	into #prónostico_mensual

	from CP_Pronóstico_AP_JGH
	where [D var] is null

--select * from #percentil75_SemanalSAP where centro='San Bernardo'

--select * from CP_Pronóstico_AP_JGH where centro='Puerto Montt'
-------------------------------------------------------------------------------------------------------------------------
...
(
	select  
		...
		PreIngresoEP			= case when Estado in ('box_atencion','espera_box_atencion') and ca.Clasif in ('Preingreso EP') then 1.0 else 0.0 end,
		Ingreso					= case when Estado in ('box_atencion','espera_box_atencion') and ca.Clasif in ('Ingreso') AND ca.TipoSiniestro_Origen not in ('Enfermedad Profesional') then 1.0 else 0.0 end,
		IngresoEP				= case when Estado in ('box_atencion','espera_box_atencion') and ca.Clasif in ('Ingreso') AND ca.TipoSiniestro_Origen= ('Enfermedad Profesional')then 1.0 else 0.0 end,
		ControlEspontaneo		= case when Estado in ('box_atencion','espera_box_atencion') and ca.Clasif in ('Control Espontaneo') then 1.0 else 0.0 end,
		ControlAgendado			= case when Estado in ('box_atencion','espera_box_atencion') and ca.Clasif in ('Control Agendado') then 1.0 else 0.0 end,
		TareasAdministrativas	= case when Estado in ('box_atencion','espera_box_atencion') and ca.Clasif in ('Tareas Administrativas') then 1.0 else 0.0 end
...
-- select * from #vw_evolucionsemanal where Rut_Paciente='10031188-7';
-- select * from CP_SGP
...
--
--- select * from #Atenciones where Rut_Paciente='10031188-7'

-- select datepart(month, Fecha) as mes,count(1) as N, Clasif from #Atenciones where centro='San Bernardo' group by datepart(month, Fecha), Clasif order by mes, Clasif asc
-------------------------------------------------------------------------------------------------------------------------
...
select
	...
	,PreIngresoEP= case when Clasif = 'PreIngreso EP' then 1.0 else 0.0 end
	,Ingreso = case when Clasif = 'Ingreso' AND TipoSiniestro_Origen not in ('Enfermedad Profesional') then 1.0 else 0.0 end
	,IngresoEP = case when Clasif = 'Ingreso' AND TipoSiniestro_Origen = ('Enfermedad Profesional') then 1.0 else 0.0 end
	,ControlAgendado = case when Clasif = 'Control Agendado' then 1.0 else 0.0 end
	,ControlEspontaneo = case when Clasif = 'Control Espontaneo' then 1.0 else 0.0 end
	,TareasAdministrativas = case when Clasif='Tareas Administrativas' then 1.0 else 0.0 end
	...
	where #CentrosSGP.centro is null and Fecha_Inicio >= dateadd(week, -@semana,getdate())
...
SELECT
	...
	,sum(PreIngresoEP*28)+sum(Ingreso*20.6)+sum(IngresoEP*37)+sum(ControlEspontaneo*15)+sum(TareasAdministrativas*6) as Tiempo_DmdVar
	,sum(ControlAgendado*15) as Tiempo_dmdControles
...
SELECT sum (TiempoDemandaVariableSAP) as T_Var, sum(TiempoControlesSAP)as T_Cont, sum(CantidadControlesSAP) as Q_Controles , sum(CantidadDemandaVariableSAP) as Q_Variable
, mes= datepart(month,FECHA) FROM #VW_Z_PERCENTILES1 where Centro='San Bernardo' group by datepart(month,FECHA)

select sum(Tiempo_DmdVar) as T_Var, sum(cast(Tiempo_dmdControles as float)) as T_Controles, sum(TotalControlesAtencion) as Q_contr,sum(TotalDemandaVariableAtencion) as Q_var, datepart(month,Fecha)  
from #vw_evolucionsemanalunion2 
where centro='San Bernardo' group by datepart(month,Fecha)
order by datepart(month,Fecha)

SELECT sum (TiempoDemandaVariableSAP) as T_Var, sum(TiempoControlesSAP)as T_Cont, sum(CantidadControlesSAP) as Q_Controles , sum(CantidadDemandaVariableSAP) as Q_Variable
, mes= datepart(month,FECHA) FROM #VW_Z_PERCENTILES1 where Centro='Puerto Montt' group by datepart(month,FECHA)

select sum(Tiempo_DmdVar) as T_Var, sum(Tiempo_dmdControles) as T_Controles, sum(TotalControlesAtencion) as Q_contr,sum(TotalDemandaVariableAtencion) as Q_var, datepart(month,Fecha)  
from #vw_evolucionsemanalunion2 
where centro='Puerto Montt' group by datepart(month,Fecha)
order by datepart(month,Fecha)
*/
---------------------------------------------------------------------------------------------------------------------------------
...
select
	...
	TotalDemandaVariableAtencion		= case when TotalDemandaVariableAtencion is null then 0.0 else TotalDemandaVariableAtencion END,
	TotalControlesAtencion				= case when TotalControlesAtencion is null then 0.0 else TotalControlesAtencion END,

	Tiempo_DmdVar						= case when Tiempo_DmdVar	   is null then 0.0 else Tiempo_DmdVar	   END ,
	Tiempo_dmdControles					= case when  Tiempo_dmdControles	  is null then 0.0 else Tiempo_dmdControles  END

	into #unionSGPHCsinnull
	from #unionSGPHC
	where fecha >= dateadd(week, -(@semana),getdate()) and fecha <= getdate()
);
-- select * from #unionSGPHCsinnull where Centro='Alameda' and DiaSem='Lunes' and mediahora='12:00:00';
/*select * from #unionSGPHCsinnull where Centro='San Bernardo'

select aa.Centro, sum(aa.Cont) as controles 
from (select Centro, DiaSem, MediaHora, avg(TotalControlesAtencion) as Cont from #unionSGPHCsinnull where Centro='San Bernardo' group by Centro, DiaSem, MediaHora) aa
group by Centro

select Centro, sum(TotalControlesAtencion) as controles from #unionSGPHCsinnull where Centro='San Bernardo' group by Centro
*/
---------------------------------------------------------------------------------------------------------------------------------
----------------------------OBTENER PERCENTIL 75 DE LOS TIEMPOS DE ATENCION POR CENTRO, DIA Y MEDIAHORA -------------------------
---------------------------------------------------------------------------------------------------------------------------------
drop table if exists #percentil75_mediahora2;
(
select distinct
	Centro, 
	DiaSem, 
...
select 
	...
	TotalDemandaVariableAtencion_per	= avg(TotalDemandaVariableAtencion),
	TotalControlesAtencion_per			= avg(TotalControlesAtencion) 
	into #percentil75_mediahora
	from #unionSGPHCsinnull
	Group by Centro, Diasem, Mediahora--, TotalDemandaVariableAtencion, TotalControlesAtencion
);

--select * from #percentil75_mediahora where Centro='Alameda' and DiaSem='Lunes' and MediaHora='12:00:00';
...
select 	Centro, 
	...
	Devest_variable	= isnull(STDEV(Tiempo_DmdVar/30),0),
	Devest_controles = 	isnull(STDEV(Tiempo_dmdControles/30),0)
--into #Devest_bloque
into #DEVS_M_AP
from #unionSGPHCsinnull
...
--select Centro, sum(PorcentajeTotalControlesAtencion_per) as Per_C, sum(PorcentajeTotalDemandaVariableAtencion_per) as Per_V from #porcentaje_mediahora 
--where Centro='San Bernardo' group by Centro
...
(
	...
	--Tiempos
	PorcentajeTotalDemandaVariableAtencion_per	*persap.Variable	*1.00 as DemandaTVariableAtencion, ---Arreglar
	PorcentajeTotalControlesAtencion_per		*persap.Controles			*1.00 as DemandaTControlesAtencion
	--Cantidades
	...
	left join
		#prónostico_mensual persap
		on
		#porcentaje_mediahora.centro=persap.Centro
);
--
--select * from #demandahoraria;
--select centro, mes, sum(demandatcontrolesatencion) from #demandahoraria where centro='Puerto Montt' group by centro, mes;
/*
select Centro, sum(DemandaTControlesAtencion) T_cont, sum(DemandaQControlesAtencion) as Q_cont from #demandahoraria
where Centro='San Bernardo'
group by Centro

select * from #percentil75_SemanalSAP where Centro='San Bernardo'
*/

select * from CP_MallaControles_AP_JGH
-------------------------------------------------------------------------------------------------------------------------
...
drop table if exists #CP_detalleDMD_pronostico_JGH---#Union_2
select aa.*, bb.Devest_variable, bb.Devest_controles
into #CP_detalleDMD_pronostico_JGH--#Union_2 
from #union1 aa
left join #DEVS_M_AP bb on aa.Centro=bb.Centro and aa.DiaSem=bb.DiaSem and aa.MediaHora=bb.MediaHora
where Mes is not null


truncate table CP_detalleDMD_pronostico_JGH
INSERT INTO CP_detalleDMD_pronostico_JGH
SELECT * FROM #CP_detalleDMD_pronostico_JGH
GO

--select * from #Union_2 order by TDV3 desc
--  where Centro in ('Alameda','Ancud','Angol', 'Concepción', 'La Florida','Las Condes')
/*
select sum(TDV3),sum(TCA3) from #CP_detalleDMD_pronostico_JGH where centro='Puerto Montt'group by Centro
select * from CP_detalleDMD_pronostico_JGH where centro='Puerto Montt'
--****************************************************************************************************************************
;drop table if exists #6Meses
select f2.*, Lapse = '6 Meses' into #6Meses from #union_2 f2 order by centro, diasem,mediahora
;
...
select * from #Union_2 where Centro='San Bernardo' 
select Centro, sum(QCA3) as Q_controles, sum(TCA3) as tiempoC from #Union_2 where Centro='San Bernardo' group by Centro order by Centro
select Centro, CantidadDemandaVariableSAP_per, CantidadControlesSAP_per from #percentil75_SemanalSAP order by Centro 
*/